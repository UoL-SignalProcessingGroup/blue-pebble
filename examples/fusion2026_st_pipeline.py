"""Pipeline helpers for the FUSION 2026 Stone Soup figure notebook."""
...
def build_config(seed: int = 2000) -> dict:
    """Create a reproducible notebook configuration dictionary."""
    np.random.seed(seed)
...
def build_platform(cfg: dict) -> TowedArrayPlatform:
    """Build and propagate the towed-array platform."""
    sim = cfg["sim"]
    ship = cfg["ship"]
    array = cfg["array"]
...
def build_target_truths(
    ...
    """Generate target state paths and relative-bearing truth paths."""
    sim = cfg["sim"]
...
def build_simulator(
    ...
    """Build simulator, signal/noise models, beamformer, and steering."""
    ...
    # Keep same behavior: use first target's signal settings for simulator signal model.
...
def run_tracking(
    ...
    all_detections: list,
    relative_bearing_truth: GroundTruthPath,
) -> Track:
    """Run PDA tracking over detections using bearing-only measurements."""
    ...
    initial_bearing = relative_bearing_truth[0].state_vector[0] + np.random.normal(
        0, np.deg2rad(2)
    )
...
def generate_stonesoup_detections(
    ...
    """Generate synthetic Stone Soup detections from bearing truth and clutter."""
...
def _add_arrowhead_at_index(
    ...
    """Add a triangular arrowhead at index ``i`` in data coordinates."""
...
def build_world_figure(
    ...
    """Build world-view figure with platform/target trajectories."""
    fig = go.Figure()
    ...
    for i in range(10, len(plat_x), 30):
        _add_arrowhead_at_index(fig, plat_x, plat_y, i, "black", size=0.3)
    for i in range(10, len(tgt_x), 30):
        _add_arrowhead_at_index(fig, tgt_x, tgt_y, i, "#ff4141", size=0.3)
    ...
    fig.update_layout(
        ...
        showlegend=True,
        legend=dict(
            x=0.5, y=-0.25, xanchor="center", orientation="h", yanchor="bottom"
        ),
        plot_bgcolor="white",
...
def build_tracker_figure(
    ...
    all_detections: list,
    track: Track,
    relative_bearing_ground_truth: GroundTruthPath,
) -> go.Figure:
    """Build 3-panel beamforming+detection+track figure."""
    det_x = []
    det_y = []
...
def build_plugin_vs_ss_figure(
    ...
    fig.add_trace(
        go.Scatter(
            x=ss_det_x, y=ss_det_y, name="Detection", showlegend=True, **scatter_style
        ),
        row=1,
        col=2,
    )
...
def print_summary(cfg: dict) -> None:
    """Print scenario summary for quick run inspection."""
    tab = " " * 4
    ...
    print(
        f"Simulation Duration: {cfg['total_duration_s']} s "
        f"across {cfg['sim']['num_steps']} timesteps"
    )
    print(f"Timestep Interval: {cfg['sim']['time_interval'].total_seconds()} s")
    print(
        f"Array: {cfg['array']['num_sensors']} sensors, "
        f"{cfg['array']['sensor_spacing']} m spacing"
    )
    print(f"Signal Sampling Rate: {cfg['signal']['sampling_rate_hz']} Hz")
    print(
        "Ambient Noise Level: "
        f"{20 * np.log10(cfg['ambient_noise']['amplitude_upa']):.1f} dB re 1 µPa"
    )
    ...
    for target_idx, target_cfg in enumerate(cfg["targets"]):
        ...
        print(
            f"{tab}Amplitudes: {20 * np.log10(target_cfg['amplitudes_upa'])} "
            "dB re 1 µPa"
        )
        print(
        ...
        print(
            f"{tab}Noise Level: {20 * np.log10(target_cfg['noise_amplitude_upa']):.1f} "
            "dB re 1 µPa"
        )
