"""Unit tests for the HostPlatform / TowedArraySensor follower model helpers."""
...
def _load_platform_and_sensors(monkeypatch):
    """Load the real HostPlatform + sensors.base + sensors.towed_array modules.

    Returns (platform_base, sensors_base, towed_array) — the three real, un-mocked
    modules under test, wired together under the fake stonesoup shim above.
    """
    _install_fake_stonesoup_towedarray(monkeypatch)
    install_repo_package(monkeypatch, "bluepebble", "bluepebble")
    install_repo_package(monkeypatch, "bluepebble.platform", "bluepebble/platform")
    install_repo_package(monkeypatch, "bluepebble.sensors", "bluepebble/sensors")

    platform_base = load_package_module_from_repo(
        "bluepebble/platform/base.py", "bluepebble.platform.base"
    )
    sensors_base = load_package_module_from_repo(
        "bluepebble/sensors/base.py", "bluepebble.sensors.base"
    )
    towed_array = load_package_module_from_repo(
        "bluepebble/sensors/towed_array.py", "bluepebble.sensors.towed_array"
    )
    return platform_base, sensors_base, towed_array
...
def test_follower_model_uses_default_direction_when_positions_overlap(monkeypatch) -> None:
    ...
    _, _, module = _load_platform_and_sensors(monkeypatch)
...
# ---------------------------------------------------------------------------
# HostPlatform + TowedArraySensor – helpers
# ---------------------------------------------------------------------------
...
def _make_host(
    ...
    **host_extra,
    ...
    """Return a ``HostPlatform`` wired to a fake constant-velocity movement_controller."""
    GroundTruthState = towed_array_module.GroundTruthState
    MovingMovable = towed_array_module.MovingMovable
    ...
    kwargs = dict(movement_controller=controller, position_mapping=[0, 2, 4])
    kwargs.update(host_extra)
    return platform_base_module.HostPlatform(**kwargs)
...
def _make_platform(
    ...
    **extra,
    ...
    """Return a ``(host, sensor)`` pair: a HostPlatform with a TowedArraySensor attached."""
    host = _make_host(
        platform_base_module,
        towed_array_module,
        host_velocity_x=host_velocity_x,
        host_z=host_z,
    ...
    kwargs.update(extra)
    sensor = towed_array_module.TowedArraySensor(**kwargs)
    return host, sensor


# ---------------------------------------------------------------------------
# HostPlatform – depth, attach/move cascade
# ---------------------------------------------------------------------------


def test_host_depth_reads_z_from_position(monkeypatch) -> None:
    """HostPlatform.depth should be the z-component of position (ship vs. sub)."""
    platform_base, _, towed_array = _load_platform_and_sensors(monkeypatch)
    host = _make_host(platform_base, towed_array, host_z=150.0, array_depth_m=150.0)

    assert host.depth == 150.0


def test_host_move_cascades_to_attached_sensors(monkeypatch) -> None:
    """move() should advance the host and call _on_host_moved on every attached sensor."""
    platform_base, _, towed_array = _load_platform_and_sensors(monkeypatch)
    host = _make_host(platform_base, towed_array)

    calls = []

    class _DummySensor:
        def _on_host_moved(self, timestamp, **kwargs):
            calls.append(timestamp)

    host.attach(_DummySensor())
    host.move(_T1)

    assert calls == [_T1]
    assert host.state.timestamp == _T1


def test_sensor_move_delegates_to_host(monkeypatch) -> None:
    """Sensor.move() should be a pure delegate to host.move()."""
    platform_base, _, towed_array = _load_platform_and_sensors(monkeypatch)
    host, sensor = _make_platform(platform_base, towed_array, num_sensors=1)

    sensor.move(_T1)

    assert host.state.timestamp == _T1
    assert sensor.platform_history[-1].timestamp == _T1


# ---------------------------------------------------------------------------
# TowedArraySensor – construction
# ---------------------------------------------------------------------------


def test_platform_constructs_and_initialises_sensors(monkeypatch) -> None:
    """TowedArraySensor should create the correct number of towed sensors."""
    platform_base, _, towed_array = _load_platform_and_sensors(monkeypatch)

    _, sensor = _make_platform(platform_base, towed_array, num_sensors=3)

    assert len(sensor.towed_sensors) == 3
    # Initial state should be recorded in history
    assert len(sensor.platform_history) == 1
    assert sensor.platform_history[0].timestamp == _T0


def test_platform_constructs_with_explicit_velocity_mapping(monkeypatch) -> None:
    """When the host's velocity_mapping is provided it should be used instead of the fallback."""
    platform_base, _, towed_array = _load_platform_and_sensors(monkeypatch)

    host, _sensor = _make_platform(
        platform_base, towed_array, host_extra={"velocity_mapping": [1, 3, 5]}
    )

    assert host.velocity_mapping == [1, 3, 5]
    assert list(host.resolved_velocity_mapping()) == [1, 3, 5]


# ---------------------------------------------------------------------------
# HostPlatform – resolved_velocity_mapping
# ---------------------------------------------------------------------------
...
def test_resolved_velocity_mapping_returns_explicit_when_set(monkeypatch) -> None:
    ...
    platform_base, _, towed_array = _load_platform_and_sensors(monkeypatch)
    host = _make_host(platform_base, towed_array, velocity_mapping=[2, 4, 6])

    assert list(host.resolved_velocity_mapping()) == [2, 4, 6]
...
def test_resolved_velocity_mapping_falls_back_to_position_plus_one(monkeypatch) -> None:
    ...
    platform_base, _, towed_array = _load_platform_and_sensors(monkeypatch)
    host = _make_host(platform_base, towed_array)  # velocity_mapping defaults to None

    assert host.velocity_mapping is None
    assert list(host.resolved_velocity_mapping()) == [1, 3, 5]


# ---------------------------------------------------------------------------
# TowedArraySensor – _initialise_sensor_array edge cases
# ---------------------------------------------------------------------------
...
def test_initialise_sensor_array_uses_zero_velocity_fallback_heading(monkeypatch) -> None:
    ...
    _, sensor = _make_platform(
        platform_base, towed_array, num_sensors=1, host_velocity_x=0.0,
        towed_array_bow_offset_m=0.0,
    )

    assert len(sensor.towed_sensors) == 1
    # Sensor should be placed 10 m in the +x direction (opposite of -x heading)
    sensor_pos = sensor.towed_sensors[0].states[-1].state_vector.flatten()
    np.testing.assert_allclose(sensor_pos[0], -10.0, atol=1e-10)


# ---------------------------------------------------------------------------
# TowedArraySensor – _capture_platform_state
# ---------------------------------------------------------------------------
...
def test_capture_platform_state_early_returns_on_missing_host_state(monkeypatch) -> None:
    ...
    platform_base, _, towed_array = _load_platform_and_sensors(monkeypatch)
    _, sensor = _make_platform(platform_base, towed_array)

    history_before = len(sensor.platform_history)
    # _T2 has never been added to host or sensor states
    sensor._capture_platform_state(_T2)

    assert len(sensor.platform_history) == history_before


# ---------------------------------------------------------------------------
# TowedArraySensor – move (via HostPlatform.move cascade)
# ---------------------------------------------------------------------------


def test_move_advances_host_and_sensors_and_records_state(monkeypatch) -> None:
    """host.move() should update every movable and append a new PlatformState."""
    platform_base, _, towed_array = _load_platform_and_sensors(monkeypatch)
    host, sensor = _make_platform(platform_base, towed_array, num_sensors=2)

    assert len(sensor.platform_history) == 1

    host.move(_T1)

    assert len(sensor.platform_history) == 2
    assert sensor.platform_history[1].timestamp == _T1
    # Host should have advanced by one velocity step
    host_pos_after = sensor.platform_history[1].host.state.state_vector[0, 0]
    np.testing.assert_allclose(host_pos_after, 5.0, atol=1e-10)


# ---------------------------------------------------------------------------
# TowedArraySensor – get_platform_state_at
# ---------------------------------------------------------------------------
...
def test_get_platform_state_at_returns_state_for_matching_timestamp(monkeypatch) -> None:
    ...
    platform_base, _, towed_array = _load_platform_and_sensors(monkeypatch)
    host, sensor = _make_platform(platform_base, towed_array)
    host.move(_T1)

    result = sensor.get_platform_state_at(_T1)
...
def test_get_platform_state_at_returns_none_for_unknown_timestamp(monkeypatch) -> None:
    ...
    assert sensor.get_platform_state_at(_T2) is None


# ---------------------------------------------------------------------------
# TowedArraySensor – get_host_state_at
# ---------------------------------------------------------------------------
...
def test_get_host_state_at_returns_state_for_matching_timestamp(monkeypatch) -> None:
    ...
    result = sensor.get_host_state_at(_T1)
...
def test_get_host_state_at_returns_none_for_unknown_timestamp(monkeypatch) -> None:
    ...
    assert sensor.get_host_state_at(_T2) is None


# ---------------------------------------------------------------------------
# TowedArraySensor – get_sensor_states_at
# ---------------------------------------------------------------------------
...
def test_get_sensor_states_at_returns_all_sensor_states(monkeypatch) -> None:
    ...
    states = sensor.get_sensor_states_at(_T1)
...
def test_get_sensor_states_at_returns_none_when_sensor_missing(monkeypatch) -> None:
    ...
    platform_base, _, towed_array = _load_platform_and_sensors(monkeypatch)
    _, sensor = _make_platform(platform_base, towed_array, num_sensors=2)

    # _T2 has never been added to any sensor
    assert sensor.get_sensor_states_at(_T2) is None


# ---------------------------------------------------------------------------
# TowedArraySensor – host_path
# ---------------------------------------------------------------------------
...
def test_host_path_returns_none_when_no_states(monkeypatch) -> None:
    ...
    host.movement_controller.states.clear()

    assert sensor.host_path is None


# ---------------------------------------------------------------------------
# TowedArraySensor – sensor_paths
# ---------------------------------------------------------------------------
...
def test_sensor_paths_returns_empty_list_when_no_sensors(monkeypatch) -> None:
    ...
    platform_base, _, towed_array = _load_platform_and_sensors(monkeypatch)
    _, sensor = _make_platform(platform_base, towed_array, num_sensors=0)

    assert sensor.sensor_paths == []
...
def test_sensor_paths_returns_empty_array_for_sensor_with_no_states(monkeypatch) -> None:
    ...
    platform_base, _, towed_array = _load_platform_and_sensors(monkeypatch)
    _, sensor = _make_platform(platform_base, towed_array, num_sensors=1)

    sensor.towed_sensors[0].states.clear()
    paths = sensor.sensor_paths
...
def test_repr_contains_configuration_parameters(monkeypatch) -> None:
    ...
    platform_base, _, towed_array = _load_platform_and_sensors(monkeypatch)
    _, sensor = _make_platform(platform_base, towed_array, num_sensors=4, cable_length_m=20.0)
...
def test_initialise_sensor_array_raises_valueerror_when_states_empty(monkeypatch) -> None:
    """IndexError on host.states[0] must be re-raised as ValueError with a clear message."""
    platform_base, _, towed_array = _load_platform_and_sensors(monkeypatch)
    MovingMovable = towed_array.MovingMovable
    ...
    host = platform_base.HostPlatform(movement_controller=controller, position_mapping=[0, 2, 4])

    with pytest.raises(ValueError, match="initial state"):
        towed_array.TowedArraySensor(
            host=host,
            num_sensors=1,
            cable_length_m=10.0,
...
def test_initialise_sensor_array_exact_boundary_does_not_raise(monkeypatch) -> None:
    ...
    _, sensor = _make_platform(
        platform_base, towed_array, num_sensors=1, cable_length_m=10.0,
        array_depth_m=10.0, host_z=0.0, towed_array_bow_offset_m=0.0,
    )
...
def test_initialise_sensor_array_accepts_leader_with_state_vector_but_no_states(
    ...
    platform_base, _, towed_array = _load_platform_and_sensors(monkeypatch)
    GroundTruthState = towed_array.GroundTruthState
    MovingMovable = towed_array.MovingMovable

    # Build a host + sensor, then replace sensor 0 with an object that has only
    # state_vector (no .states list), and re-run _initialise_sensor_array with
    # num_sensors=2 so that sensor 1 uses that patched object as its leader —
    ...
    p = towed_array.TowedArraySensor(
        host=host,
        num_sensors=1,
        cable_length_m=10.0,
        sensor_spacing_m=5.0,
...
def test_capture_platform_state_skips_when_sensor_states_missing_at_timestamp(
    ...
    new_sv = host.movement_controller.states[-1].state_vector.copy()
    host.movement_controller.states.append(GroundTruthState(state_vector=new_sv, timestamp=_T2))

    history_len = len(sensor.platform_history)
    sensor._capture_platform_state(_T2)

    # Sensors have no state at _T2 → early return → history unchanged
    assert len(sensor.platform_history) == history_len
...
def test_capture_platform_state_records_correct_heading(monkeypatch) -> None:
    ...
    _, sensor = _make_platform(platform_base, towed_array, num_sensors=1, host_velocity_x=5.0)

    heading = sensor.platform_history[0].host.heading_rad
...
def test_capture_platform_state_uses_non_default_reference_sensor(monkeypatch) -> None:
    ...
    platform_base, _, towed_array = _load_platform_and_sensors(monkeypatch)
    _, sensor = _make_platform(platform_base, towed_array, num_sensors=3, reference_sensor_idx=2)

    state = sensor.platform_history[0]
    expected_ref = sensor.towed_sensors[2].states[-1].state_vector
...
def test_array_state_combined_vector_is_hstack_of_sensor_vectors(monkeypatch) -> None:
    ...
    state = sensor.platform_history[0]
    expected = np.hstack([s.states[-1].state_vector for s in sensor.towed_sensors])
...
def test_get_sensor_states_at_returns_empty_list_when_no_sensors(monkeypatch) -> None:
    ...
    result = sensor.get_sensor_states_at(_T0)
...
def test_multiple_moves_accumulate_platform_history(monkeypatch) -> None:
    """Each call to host.move() should append exactly one entry to platform_history."""
    platform_base, _, towed_array = _load_platform_and_sensors(monkeypatch)
    host, sensor = _make_platform(platform_base, towed_array, num_sensors=1)

    assert len(sensor.platform_history) == 1  # initial capture

    host.move(_T1)
    assert len(sensor.platform_history) == 2

    host.move(_T2)
    assert len(sensor.platform_history) == 3
    assert sensor.platform_history[-1].timestamp == _T2
