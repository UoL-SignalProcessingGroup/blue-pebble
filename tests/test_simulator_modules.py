...
def test_hold_last_finite_delays_fills_gaps_from_nearest_finite_knot(monkeypatch) -> None:
    """Non-finite delays should be replaced by the nearest finite delay for that sensor."""
    _base, _discrete, continuous = _load_simulator_modules(monkeypatch)

    inf = np.inf
    delays = np.array(
        [
            # sensor 0: gap in the middle -> held from the previous knot
            # sensor 1: gap at the start  -> held from the next knot (no previous exists)
            # sensor 2: never has an arrival -> falls back to zero
            # sensor 3: fully finite -> untouched
            [1.0, inf, inf, 4.0],
            [inf, inf, inf, 5.0],
            [inf, 3.0, inf, 6.0],
            [2.0, 3.5, inf, 7.0],
    ...
    held = continuous._hold_last_finite_delays(delays)

    assert np.isfinite(held).all()
    np.testing.assert_array_equal(held[:, 0], [1.0, 1.0, 1.0, 2.0])
    np.testing.assert_array_equal(held[:, 1], [3.0, 3.0, 3.0, 3.5])
    np.testing.assert_array_equal(held[:, 2], [0.0, 0.0, 0.0, 0.0])
    np.testing.assert_array_equal(held[:, 3], [4.0, 5.0, 6.0, 7.0])


def test_hold_last_finite_delays_returns_finite_input_unchanged(monkeypatch) -> None:
    """A fully finite history must pass through untouched, and never alias the input."""
    _base, _discrete, continuous = _load_simulator_modules(monkeypatch)

    delays = np.array([[1.0, 2.0], [3.0, 4.0]])
    held = continuous._hold_last_finite_delays(delays)

    np.testing.assert_array_equal(held, delays)
    held[0, 0] = 99.0
    assert delays[0, 0] == 1.0, "helper must not return a view onto its input"


class _NoArrivalModel:
    """Signal model stub for the no-arrival tests."""
    ...
    def compute_stft(self, state):
        """Return a flat source spectrum over three frames."""
        _ = state
        return np.ones((3, 3), dtype=np.complex64), np.array([0.0, 0.5, 1.0]), 2, np.ones(4)

    def stft_geometry(self):
        """Return STFT geometry matching ``compute_stft``."""
        return 3, np.array([0.0, 0.5, 1.0], dtype=np.float64), 2, np.ones(4), 3

    def get_source_waveform(self, state):
        """Return a flat source waveform."""
        _ = state
        return np.ones(6, dtype=np.complex64)


def _no_arrival_propagate(platform_state, target_state, freqs):
    """Sensor 1 has no eigenray: zero channel gain and an infinite delay."""
    _ = (platform_state, target_state)
    transfer = np.ones((2, len(freqs)), dtype=np.complex64)
    transfer[1, :] = 0.0
    return transfer, np.array([0.25, np.inf])


def test_build_target_histories_holds_non_finite_delays_finite(monkeypatch) -> None:
    """An infinite delay from the propagation model must not reach the delay history.

    This is asserted on the history directly rather than on rendered signals, because the
    test harness stubs ``inverse_stft`` to return zeros -- which would make a finiteness
    check on ``stft_interp`` output pass vacuously.
    """
    _base, _discrete, continuous = _load_simulator_modules(monkeypatch)
    t0 = datetime(2026, 1, 1, 12, 0, 0)
    timestamps = [t0 + timedelta(seconds=i) for i in range(3)]

    simulator = continuous.ContinuousSTFTPassiveSonarArraySimulator(
        platform=_FakePlatform(timestamps, num_sensors=2),
        propagation_model=_prop_model(_no_arrival_propagate),
        signal_models=[_NoArrivalModel()],
    )
    ctx = simulator._build_common_context(timestamps, [_NoArrivalModel()])
    histories = simulator._build_target_histories(
        ctx,
        [_FakePath(states=[_FakeState(ts) for ts in timestamps])],
        [_NoArrivalModel()],
    )

    tau_hist = histories[0].tau_hist
    assert np.isfinite(tau_hist).all(), "no-arrival delays must be held finite"
    # Sensor 1 never has an arrival, so it falls back to zero; sensor 0 keeps its delay.
    np.testing.assert_array_equal(tau_hist[:, 0], [0.25, 0.25, 0.25])
    np.testing.assert_array_equal(tau_hist[:, 1], [0.0, 0.0, 0.0])
    # The zero channel gain is what silences the sensor, and must be left alone.
    assert np.all(histories[0].H_hist[:, 1, :] == 0)


def test_no_arrival_delays_silence_a_sensor_instead_of_producing_nan(monkeypatch) -> None:
    """An infinite delay means no arrival: the sensor goes silent rather than NaN.

    Only the synthesis paths that do a real inverse transform are exercised here.
    ``stft_interp`` is covered by the delay-history test above instead, since the harness
    stubs its ``inverse_stft``.
    """
    _base, _discrete, continuous = _load_simulator_modules(monkeypatch)
    t0 = datetime(2026, 1, 1, 12, 0, 0)
    timestamps = [t0 + timedelta(seconds=i) for i in range(3)]
    platform = _FakePlatform(timestamps, num_sensors=2)
    path = _FakePath(states=[_FakeState(ts) for ts in timestamps])

    def check(raw, label):
        assert np.isfinite(raw).all(), f"{label} produced non-finite sensor signals"
        assert np.all(raw[1, :] == 0), f"{label}: the no-arrival sensor should be silent"
        assert np.any(raw[0, :] != 0), f"{label}: the arriving sensor should carry signal"
    ...
    for mode in ("wola_interp", "cola"):
        ...
        for _timestamp, data in simulator.sensor_data_gen():
            check(np.asarray(next(iter(data)).raw_signals), f"mode={mode}")

    fractional = continuous.ContinuousFractionalDelayPassiveSonarArraySimulator(
        platform=platform,
        propagation_model=_prop_model(_no_arrival_propagate),
        signal_models=[_NoArrivalModel()],
        ground_truth_paths=[path],
    ...
    for _timestamp, data in fractional.sensor_data_gen():
        check(np.asarray(next(iter(data)).raw_signals), "fractional delay")
