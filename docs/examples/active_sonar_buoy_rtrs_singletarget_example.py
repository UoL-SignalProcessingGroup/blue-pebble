"""
==============================================
Active Sonar Single-Target Example (RTRS)
==============================================

This example demonstrates the monostatic active sonar pipeline using the RTRS two-hop
Fourier-synthesis method: a stationary omnidirectional sonobuoy pings a single moving
target, and the round-trip echo is synthesised in the frequency domain from the 3-D
``rtrs`` ray tracer's transfer function ``H(f)`` rather than from discrete Bellhop
eigenray pairs. It covers:
...
- Running ``RtrsActiveSonarSimulatorOmni``: ``propagate_spectrum`` is queried directly at
  every native FFT bin in the pulse's occupied band (no sparse sampling or interpolation --
  benchmarking showed rtrs's cost is dominated by ray/beam-tracing geometry setup, not by
  the number of frequencies requested), combined via acoustic reciprocity and a
  ``ConstantTargetStrength`` scattering model.
...
— required so the sonar-equation verification (which assumes spherical TL =
20*log10(R)) is valid.

Note on detection: the matched-filter mainlobe here is still noticeably wider than
Bellhop's eigenray synthesis produces (hundreds of samples here, vs tens for Bellhop).
With frequency-domain interpolation removed, this reflects real multipath spreading in
the medium rather than any interpolation artifact, so the CFAR ``num_guard_cells`` below
is set much larger than the Bellhop example's — otherwise the training cells sample
mainlobe leakage rather than true noise and the detector never fires.
...
from bluepebble.models.scattering import ConstantTargetStrength
from bluepebble.platform import OmniSonobuoyPlatform
from bluepebble.signal.active import LFMSignal
from bluepebble.simulator import RtrsActiveSonarSimulatorOmni
...
target_params = {
    "start_vector": np.array([2000.0, 3.0, 0.0, 0.0, -80.0, 0.0]),
...
det_params = {
    ...
    "cfar_detector": {
        # Guard cells sized to clear real multipath spreading in this scenario (see module
        # docstring), not just the ~25-sample resolution cell that suffices for Bellhop.
        "num_guard_cells": 800,
        "num_training_cells": 100,
        "rank": 75,              # 75th percentile of 200 training cells
        "threshold_factor": 8,
    },
...
# Constant SSP + flat bathymetry are required for the sonar-equation
# verification below (spherical TL = 20*log10(R)) to be valid; it's
# auto-skipped for this non-constant-SSP run (see the `isinstance(ssp,
# Constant)` gate further down), matching the Bellhop example's behaviour.

prop_model = rtrsAcousticPropagationModel(ssp=ssp, bathymetry=bathymetry, step_m=20.0)
scattering_model = ConstantTargetStrength(target_strength_db=target_params["target_strength_db"])
...
simulator = RtrsActiveSonarSimulatorOmni(
    platform=buoy,
...
# Skipped automatically when a non-constant SSP is used, because refraction,
# ducting, and convergence zones break the spherical-spreading assumption and
...
if isinstance(ssp, Constant):
    ...
    for i, (_, sensor_data_set) in enumerate(all_sensor_data):
        ...
        # A_received = channel_out × channel_back × A_target (no A_signal factor).
...
out_dir = Path(__file__).resolve().parent / "figs_active_rtrs"
out_dir.mkdir(parents=True, exist_ok=True)

for fname, fig in [
    ("as_rtrs_world.pdf", fig_world),
    ("as_rtrs_range_detections.pdf", fig_range),
    ("as_rtrs_mf_waterfall.pdf", fig_mf),
    ("as_rtrs_mf_scatter.pdf", fig_mf_scatter),
    ("as_rtrs_sonar_eq.pdf", fig_sonar_eq),
]:
    if fig is None:
        continue
...
print()
print("=" * 60)
print("RTRS Active Sonar Scenario Summary")
print("=" * 60)
