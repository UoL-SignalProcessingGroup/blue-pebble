...
det_params = {
    ...
    "cfar_detector": {
        # Guard cells sized to clear the multipath spreading in this scenario (see the
        # module docstring), not just the ~25-sample resolution cell that suffices for Bellhop.
        "num_guard_cells": 800,
...
# Ground-truth 3-D slant ranges at each ping (sonobuoy hydrophone to target).
gt_ranges = []
buoy_xy = buoy.position_3d.flatten()[:2]
buoy_z = -buoy.hydrophone_depth_m
...
# A constant SSP and flat bathymetry are needed for the sonar-equation verification below;
# it is auto-skipped for this non-constant-SSP run.
...
# automatically for a non-constant SSP, where refraction breaks that assumption.
