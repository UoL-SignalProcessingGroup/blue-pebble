"""
=====================================================
Active Sonar Multi-Target Return-Time Verification (RTRS)
=====================================================
...
expected round-trip delays, using the RTRS two-hop Fourier-synthesis method
(:class:`.RtrsActiveSonarSimulatorOmni`) rather than Bellhop eigenray pairs.

This is a **timing / geometry** check, complementary to the amplitude
(sonar-equation) check in ``active_sonar_rtrs_example.py``.
...
Verification criterion
  ...
  sample (ΔR < c/(2 fs)) confirm that ray-traced delays and range conversion
  are correct end-to-end.

The scenario uses a flat 200 m bathymetry and a constant 1500 m/s sound speed
profile so that the direct-path delay equals the geometric range divided by
*c* with no refraction corrections.
...
scattering_model = ConstantTargetStrength(target_strength_db=target_strength_db)
...
print(f"  Residuals are from rtrs's ray-tracing angular resolution, not delay→range "
      f"conversion error.")
if n_bad == 0:
...
for fname, fig in [
    ("mt_rtrs_world.pdf", fig_world),
    ("mt_rtrs_mf_envelope.pdf", fig_env),
    ("mt_rtrs_mf_waterfall.pdf", fig_waterfall),
    ("mt_rtrs_det_accuracy.pdf", fig_acc),
]:
...
print("=" * 60)
print("RTRS Multi-Target Timing Verification Summary")
print("=" * 60)
