...
class PassiveSonarArraySimulatorBase(SensorSimulator):
    ...
    def _apply_hydrophone_to_chunk(
        ...
        """Apply the hydrophone transfer functions to a time-domain sensor chunk.

        Converts the chunk to the frequency domain, multiplies by the per-element
        hydrophone transfer functions from the platform's sensor array, and converts
        back.  This ensures ambient noise added before this call passes through the
        same hydrophone response as the signal.
        ...
        """
        _num_sensors, num_samples = sensor_signals.shape
        if num_samples == 0:
            return sensor_signals.astype(np.complex64)
        frequencies_hz = np.fft.fftfreq(num_samples, d=1.0 / sampling_rate_hz)
        H = self.platform.sensor_array.transfer_functions(frequencies_hz)
    ...
    def _beamform_if_configured(
        ...
        ----------
        timestamp : datetime
            Snapshot timestamp used to query array element states.
        sensor_signals : numpy.ndarray
        ...
        steering_delays_s = self.steering_calculator.calculate(
            self.platform.sensor_array, timestamp
        )
